...
#: Canonical project URL. Kept here (rather than duplicated in `vbump init` and `vbump-convert`)
#: so both stamp scaffolded/translated config files with the same pointer back to the project.
PROJECT_URL = "https://github.com/vilarneto/vbumper"


def config_header_comment() -> str:
    """First-line comment `vbump init` and `vbump-convert` write atop every config file they
    scaffold or translate, so a future Git cloner who stumbles on `.vbump.yaml` knows what wrote
    it, which version, and where to find it."""

    from importlib.metadata import version

    return f"# Generated by Version Bumper, v{version('vbumper')}. Check {PROJECT_URL} for details."
